...
XMPP.prototype.status = function(status, message){
  var client = this.client();
...
XMPP.prototype.join = function(jid){
  var client = this.client();
...
XMPP.prototype.leave = function(jid){
  var client = this.client();
...
XMPP.prototype.groupchat = function(jid, message){
  var client = this.client();
...
XMPP.prototype.chat = function(jid, message){
  var client = this.client();
  var el = new Element('message', {
    to: jid,
    type: 'chat',
...
XMPP.prototype.topic = function(jid, topic){
  var client = this.client();
  var el = new Element('message', {
    to: jid + '/' + this.name,
    type: 'groupchat'
...
XMPP.prototype.onOnline = function(){
  ...
  this.profile(function(err, profile){
    ...
    self.roster(function(err, users){
      ...
      self.rooms(function(err, rooms){
        if (err) return self.emit('error', 'Unable to get a list of rooms.');
        self.emit('connect', profile, users, rooms);
      });
    });
  });
